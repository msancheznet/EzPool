...
if sys.version_info >= (3, 0):
	...
	from Queue import Queue
from objects import Closeable, run_object
...
class WorkerQueue(Queue):
	...
	def add(self, uri):
		''' Convenience function to add a worker to the queue through its URI

			:param str uri: A fully formed URI address
		'''
		self._add_proxy(EzProxy(uri))

	def _add_proxy(self, proxy):
		""" Add a worker to this WorkerQueue

			:param EzProxy proxy: The proxy that identifies the worker
		"""	
		uri = proxy.uri
...
class JobManager(Closeable):
	''' Job manager of workers analogous to Python's multiprocessing Pool.
	...
	def shutdown(self):
		...
		with self._mutex:
			for uri in self.workers:
				self._close_worker(uri)
			self._shut = True
		print('Job manager {} has been closed'.format(uri))
		Closeable.shutdown()
	...
	def _run_task(self, wrk_id, task):
		...
		# available
		self._running.pop(wrk_id)
		self._idle.put_nowait(worker)

	@staticmethod
	def parse_args(args=None):
		""" Return the argument parser for the job manager """
...
def _get_args_parser():
	...
	parser.add_argument('--name', '-n', nargs='?', default='jobmgr', help='Job manager name', type=str)
	parser.add_argument('--msg','-m', nargs='?', default='Job manager ready to proces jobs:', help='Message to display when object is started', type=str)
	#parser.add_argument('--nserver', '-ns',action='store_true', help='Run with name server')
	return parser

if __name__ == '__main__':
	run_object(JobManager)
